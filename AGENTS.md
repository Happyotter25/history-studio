...
- `/api/jobs` accepts kind image(default)|analysis|prompt; same token/origin checks, serial execution and cancellation. Analysis uses result.json in isolated CLI workspace; archives `output/codex-plans`. Image behavior is preserved. `npm run test:cli` required for server changes (4 tests currently).
