...
## Claude Code와 번갈아 쓸 때
Claude Code는 `CLAUDE.md`를 읽는데, 이 파일은 `AGENTS.md`를 가리킵니다. 그래서 두 도구가 같은 안내를 따릅니다.

- **작업 일지 `docs/HANDOFF.md`**: 두 도구 모두 시작할 때 이 파일을 읽고, 마칠 때 맨 위에 "한 일 · 확인한 것 · 못 한 것 · 다음 할 일"을 적습니다.
  선생님도 이 파일로 진행 상황을 볼 수 있습니다. 부탁할 때 "작업 일지 보고 다음 할 일 이어서 해 줘"라고만 해도 됩니다.
- **한 번에 한 도구만**: 한쪽 작업을 `main`에 합친 다음 다른 쪽을 시작하세요. 같은 파일을 동시에 고치면 충돌합니다.
- **시작 전 `git pull`, 끝나면 커밋·올리기** (CLI일 때). 웹(Claude Code 웹, Codex 클라우드)은 GitHub에서 바로 받아 옵니다.
