...
await pg.screenshot({path:path.join(root,'docs/studio-home.png'),fullPage:true});
await pg.click('#tabs button[data-tab=teaching]');await pg.screenshot({path:path.join(root,'docs/teaching-home.png'),fullPage:true});
...
await teaching.click('#tabs button[data-tab=materials]');await teaching.click('#material-sample');await teaching.click('#material-propose');await teaching.click('#material-confirm');
...
if(!await teaching.locator('#teaching-readiness').isVisible())await teaching.click('#teaching-summary');
...
const studio=await b.newPage({viewport:{width:1440,height:1000}});
await studio.goto('file://'+path.join(root,'index.html'));await studio.waitForSelector('body[data-ready]');
await studio.fill('#studio-script','칠천량 패전으로 조선 수군은 큰 타격을 입었습니다.\n\n이순신은 장병과 군량을 모으며 수군을 재건했습니다.\n\n명량에서는 좁은 해협의 지형을 이용해 싸웠습니다.');await studio.click('#studio-paragraph');await studio.waitForFunction(()=>HS.project.studio.mode==='review');
await studio.screenshot({path:path.join(root,'docs/studio-review.png'),fullPage:true});
