...
export async function generate(job, update, register){
  ...
  const dest=path.join(dir,'result.png');
    ...
    try{
      ...
      if(url.pathname.startsWith('/api/')){
        ...
        if(url.pathname==='/api/jobs'&&req.method==='POST'){
          ...
          if(active)return send(409,{error:'이미 그림을 만드는 중입니다.'});
          // 한 번에 한 장. 완료 파일은 output에 보관하고 메모리 기록은 최근 12개만 유지합니다.
          while(jobs.size>=12){const first=jobs.keys().next().value;jobs.delete(first);}
          const job={id:randomUUID(),state:'running',message:'Codex에 연결하고 있습니다…',prompt:data.prompt};jobs.set(job.id,job);active=job;
          let timer;
          ...
          Promise.resolve().then(()=>runner(job,m=>{if(job.state==='running')job.message=m;},stop=>{job.stop=stop;if(job.state!=='running')stop();})).then(async bytes=>{
            ...
            const out=path.join(root,'output','codex-images');await fs.mkdir(out,{recursive:true});await fs.writeFile(path.join(out,job.id+'.png'),bytes);
            if(job.state!=='running')return;
            job.image='data:image/png;base64,'+bytes.toString('base64');job.state='done';job.message='그림을 완성했습니다.';
          }).catch(()=>{if(job.state==='running'){job.state='error';job.message='그림을 만들지 못했습니다. CLI 이미지 도구 지원·로그인·구독 사용량을 확인하고 다시 시도해 주세요.';}}).finally(async()=>{clearTimeout(timer);active=null;if(job.dir)await fs.rm(job.dir,{recursive:true,force:true});});
        ...
        if(match){const job=jobs.get(match[1]);if(!job)return send(404,{error:'작업 기록이 없습니다.'});
          ...
          if(req.method==='GET')return send(200,{id:job.id,state:job.state,message:job.message,image:job.image});
        }
