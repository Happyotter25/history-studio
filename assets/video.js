...
(function(){
  ...
  HS.preloadImages = function(){
    return Promise.all(HS.project.scenes.filter(function(s){ return s.image; }).map(function(s){
      HS.sceneImage(s);
  ...
  HS.sceneDuration = function(s){ return Math.max(4, Math.min(40, (s.narration || '').replace(/\s/g, '').length / CPS)) + FADE; };
  ...
  function subtitle(ctx, w, h, s, local, dur){
    ...
    var span = dur - FADE, pos = Math.max(0, local - FADE * 0.5) / span * total, acc = 0, cur = chunks[chunks.length - 1];
  ...
  HS.recordCanvas = function(canvas, duration, drawAt, onTick){
    if(!window.MediaRecorder || !canvas.captureStream) return Promise.reject(new Error('이 브라우저는 영상 녹화를 지원하지 않습니다 (크롬·엣지 권장)'));
    var types = ['video/webm;codecs=vp9', 'video/webm;codecs=vp8', 'video/webm'], mime = '';
    ...
    return new Promise(function(ok, fail){
      rec.onstop = function(){ ok(new Blob(chunks, { type: 'video/webm' })); };
