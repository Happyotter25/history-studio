# 사관 스튜디오 — 역사 영상 자료 제작 도구

**완성된 대본을 넣고, 필요한 자료 목록을 검토·확정한 뒤 삽화·이미지·지도 PNG와 스토리 PPT·판서 자료를 각각 만드는 도구**입니다.

맥에서 **`사관 스튜디오 실행.command`를 더블클릭**하고 **✦ 자료 제작**을 누르세요. 자동 그림 제작 없이 쓰려면 `index.html`을 직접 열어도 됩니다. 새 브라우저에서는 이 화면부터 시작합니다. 기본 제작 흐름에는 API 키가 필요 없습니다.

![자료 제작 시작 화면](docs/materials-start.png)

## 지금 쓰는 순서

1. **완성 대본 넣기** — 원고를 붙여 넣거나 TXT 파일로 넣습니다. **명량대첩 원고 예시**에는 선생님이 제공한 원고 전체가 들어 있습니다.
2. **제작 목록 제안받기** — 원고 표현을 보존하며 자료 구간을 나눕니다. 일반 원고는 문단·인용·지명 규칙으로 초안을 만들고, 명량 예시는 13구간의 맞춤 기획을 제공합니다. AI가 사실 검증한 결과는 아닙니다.
3. 항목별 **만들기 여부, 종류, 그림 수(1~3장), 제작 지시, 판서 내용**을 수정합니다. 원고 보기에서 근거 구간을 확인할 수 있습니다. 인용문·출처·비교표·지도 지명도 고칠 수 있습니다.
4. 필요하면 **구독 AI로 기획 다듬기 → 요청서 받기**를 눌러 ZIP을 Codex·ChatGPT에 전달합니다. AI가 돌려준 `plan.json`을 불러온 뒤 다시 검토합니다. 원고·구간 ID가 다른 파일은 거절하고 원고 자체는 바꾸지 않습니다.
5. **이 목록으로 확정** — 이때 장면과 판서가 구성됩니다. 기존 자료를 대체할 때는 안내하고 되돌리기 기록을 남깁니다. 원고나 목록을 수정하면 재확정이 필요합니다.
6. **Codex로 그림 자동 제작 → 한 장 먼저 만들기** — 로컬 CLI가 구독으로 생성하고 앱에 자동으로 넣습니다. 미리보기를 확인한 뒤 **남은 그림 만들기**를 누르세요. 직접 파일로 연 경우에는 기존 **그림 주문서 받기 → 만든 그림 불러오기**를 사용합니다.
7. **자료별 받기** — 아래 버튼으로 필요한 것만 내려받습니다.

| 자료 | 결과 |
|---|---|
| 삽화·이미지 | 완료한 그림만 1920×1080 PNG ZIP. 다시 그리기 항목은 제외 |
| 지도 | 등록 지명의 위치 참고 지도 PNG ZIP. 근거 없는 이동 경로·해류·전투 배치·시대별 영토는 생성하지 않음 |
| 인용·비교 | 원고 인용문과 비교표 PNG ZIP. 번역문·숫자·출처는 검토 필요 |
| 스토리 PPT | 표지＋선택한 각 구간의 삽화 중심 슬라이드. 해당 원고는 발표자 노트에 그대로 포함. 그림이 없으면 **초안·삽화 대기** 표시 |
| 판서 | 검토한 핵심어·인과관계·정리 내용을 PNG ZIP 또는 편집 가능한 글자의 PPT로 저장 |

명량 예시의 기본 제안은 **삽화 11장, 위치 지도 1장, 인용문 3장, 비교표 1장, 판서 13장**입니다. 수량과 내용은 확정 전에 바꿀 수 있습니다.

![자료 목록 검토](docs/materials-plan.png)

## 기존 세부 도구

대본 편집·그림 후보 관리·손그림 변환·영상 도구 등은 상단 탭에 남아 있습니다. 아래는 그 세부 기능 안내입니다. 완성 대본으로 자료를 만들 때는 위 **자료 제작** 흐름을 먼저 사용하세요.
...
자료 제작에서 목록을 확정한 다음 **강의 자료 작업실**을 여세요. 기존 대본으로도 사용할 수 있습니다.
