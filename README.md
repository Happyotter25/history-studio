...
### 강의 자료 작업실에서 전체 장면 자동 제작

장면 구분을 확정하면 **전체 장면 자동 제작** 영역이 나타납니다. 공통 그림 스타일을 적고 **전체 프롬프트 + 이미지 자동 제작**을 누르면 글·판서 장면도 이미지로 전환하여 한 장면씩 만듭니다. 원고와 기존 글·판서 내용은 보존됩니다.

- **미완성 장면 채우기**: 완성된 그림은 유지하고 비어 있거나 지시가 바뀐 그림만 만듭니다. 준비된 프롬프트는 재사용합니다.
- **전체 장면 다시 만들기**: 프롬프트부터 새로 작성합니다. 교체한 그림은 이전 후보에 남깁니다.
- **실패한 장면만 다시 시도**: 실패한 장면만 이어서 처리합니다.
- **프롬프트만 먼저 작성**: 선택 범위에 자동 적용하며, 그림은 생성하지 않습니다.

장면별 진행 상태·오류와 이미지 미리보기를 확인할 수 있습니다. 중단해도 완료한 결과는 남습니다. 창을 닫으면 자동 제작은 이어지지 않으므로 다시 열어 미완성 채우기를 실행하세요. 구독 사용량에 따라 시간이 걸릴 수 있습니다. 제작 후 **다음 검토할 장면**으로 이동해 역사적 표현을 확인하고, 검토 완료한 자료를 PNG로 저장하세요.

강의 자료 작업실의 **슬라이드 PPT**는 각 슬라이드의 메모(발표자 노트)에 제목·표현 종류·슬라이드 내용(이미지는 생성 프롬프트)·해당 장면 원고·출처 및 제작 메모를 자동으로 넣습니다. PowerPoint의 메모 창에서 확인하고 수정할 수 있습니다.
